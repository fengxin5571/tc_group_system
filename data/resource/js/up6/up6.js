...
function HttpUploaderMgr()
{
	...
	this.Config = {
		...
		//文件操作相关
		, "UrlCreate"		: base_site_url+"/data/resource/up6/f_create.php"
		, "UrlPost"			: base_site_url+"/data/resource/up6/f_post.php"
        , "UrlProcess"		: base_site_url+"/data/resource/up6/f_process.php"
		, "UrlComplete"		: base_site_url+"/data/resource/up6/f_complete.php"
		, "UrlList"			: base_site_url+"/data/resource/up6/f_list.php"
		, "UrlDel"			: base_site_url+"/data/resource/up6/f_del.php"
	    //x86
        , ie: {
	...
	this.FileListMgr =
	{
		...
		, "RemoveFile": function (fileSvr)//删除文件
		{
			...
			$.ajax({
				...
				, error: function () { alert("发送删除文件信息失败！"+req.responseText);console.log(req.responseText); }
				, complete: function (req, sta) { req = null; }
			});
		}
	...
	this.GetHtmlFiles = function()
	{
		...
		acx += '<div class="files-panel" name="post_panel">\
					<div name="post_head" class="toolbar" style="position: static;">\
