...
class video_classModel{
    ...
    public function getClassList($condition){
        ...
        $param = array();
        $param['table'] = 'video_class';
        $param['where'] = $condition_str;
        $param['order']	= empty($condition['order'])?'vd_parent_id asc,vd_sort asc,vd_id asc':$condition['order'];
    ...
    private function _condition($condition){
        ...
        if ($condition['vd_parent_id'] != ''){
            $condition_str .= " and vd_parent_id = '". intval($condition['vd_parent_id']) ."'";
        }
        if ($condition['no_vd_id'] != ''){
            $condition_str .= " and vd_id != '". intval($condition['no_vd_id']) ."'";
        }
        if ($condition['vd_name'] != ''){
            $condition_str .= " and vd_name = '". $condition['vd_name'] ."'";
        }
        if ($condition['home_index'] != ''){
            $condition_str .= " and vd_id <= 7";
        }
    ...
    public function add($param){
        ...
        if (is_array($param)){
            ...
            $result = Db::insert('video_class',$tmp);
            return $result;
        }else {
            return false;
        }
    ...
    public function update($param){
        ...
        if (is_array($param)){
            ...
            $where = " vd_id = '". $param['vd_id'] ."'";
            $result = Db::update('video_class',$tmp,$where);
            return $result;
        }else {
            return false;
        }
    ...
    public function del($id){
        if (intval($id) > 0){
            $where = " vd_id = '". intval($id) ."'";
            $result = Db::delete('video_class',$where);
            return $result;
        }else {
            return false;
        }
     ...
     */
    private function _getTreeClassList($show_deep,$class_list,$deep=0,$parent_id=0,$i=0){
    ...
    public function getChildClass($parent_id){
        $all_class = $this->getClassList(array('order'=>'vd_parent_id asc,vd_sort asc,vd_id asc'));
        if (is_array($all_class)){
            ...
            foreach ($all_class as $k => $v){
                $ac_id	= $v['vd_id'];//返回的结果包括父类
                $ac_parent_id	= $v['vd_parent_id'];
