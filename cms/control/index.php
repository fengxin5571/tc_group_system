...
class indexControl extends CMSHomeControl{
	...
	public function indexOp(){
	    //导航
	    $nav_class=Model('cms_navigation');
	    $condition=array();
	    $nav_list=$nav_class->select($condition);
	    ...
	    $commend_article=$cms_article_model->getList($condition,$page=null,$order='article_id desc', $field='*', $limit='4');
	    Tpl::output('commend_article',$commend_article);
	    $condition=array('article_commend_image_flag'=>1);
	    $commend_image_article=$cms_article_model->getOne($condition,$order='article_id desc');
	    $commend_image_article['article_image']=unserialize($commend_image_article['article_image']);
	    Tpl::output('commend_image_article',$commend_image_article);
	    
	    
        Tpl::showpage('index');
	}
	
	public function indexOp1(){
