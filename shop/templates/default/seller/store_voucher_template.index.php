...
<?php if ($output['isOwnShop']) { ?>
    <a class="ncsc-btn ncsc-btn-green" style="right:100px" href="<?php echo urlShop('pointvoucher', 'index', array('store_id' => $_SESSION['store_id']));?>" target="_blank"><i class="icon-plus-sign"></i>查看活动列表</a>
...
<?php } else { ?>
    ...
    <a class="ncsc-btn ncsc-btn-green" style="right:200px" href="<?php echo urlShop('pointvoucher', 'index', array('store_id' => $_SESSION['store_id']));?>" target="_blank"><i class="icon-plus-sign"></i>查看活动列表</a>	
...
<table class="ncsc-default-table">
	...
	<tbody>
      ...
      <tr class="bd-line">
        	...
        	<!-- 代金券详细 -->
        	<span>
        	   <a class="btn-blue" href="index.php?act=store_voucher&op=templateinfo&tid=<?php echo $val['voucher_t_id'];?>"><i class="icon-th-list"></i><p>详细</p></a>
			</span>
        	<?php if ((!$val['voucher_t_giveout']) && (!$val['voucher_t_isbuild'])){//该模板没有发放过代金券时可以删除?>
        	<span> <a class="btn-blue" href="javascript:void(0)" onclick="ajax_get_confirm('<?php echo $lang['nc_ensure_del'];?>','index.php?act=store_voucher&op=templatedel&tid=<?php echo $val['voucher_t_id'];?>');"><i class="icon-trash"></i><p>删除</p></a></span>
        	<?php }?>
        </td>
      </tr>
      <?php }?>
