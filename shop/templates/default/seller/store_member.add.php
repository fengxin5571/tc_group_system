...
<div class="ncsc-form-default">
  <form id="add_form" action="<?php echo urlShop('store_member_manage', 'member_add');?>" method="post">
    ...
    <dl>
      ...
      <dd> <input type="text" name="member_name"  id="member_name"/><span></span>
        <p class="hint">留空表示默认为填写的会员手机（登录线上商城的账号）</p>
      </dd>
    </dl>
    <dl>
      <dt><i class="required">*</i>会员密码<?php echo $lang['nc_colon'];?></dt>
      <dd> <input type="password" name="member_passwd"/><span></span>
        <p class="hint">留空表示不修改密码,初始123456（登录线上商城的密码）</p>
      </dd>
    </dl>
    <dl>
...
$(document).ready(function(){
	...
	jQuery.validator.addMethod("check_member_mobile", function(value, element, params) { 
	    ...
	    if(!mobile.test(value)){
		 ...
		 $.ajax({  
		        ...
		        async:false,  
		        data:{member_mobile: $('#member_mobile').val()},  
	...
	jQuery.validator.addMethod("check_member_card", function(value, element, params) { 
		...
		$.ajax({  
	        ...
	        async:false,  
	        data:{member_card: $('#member_card').val()},  
	        success: function(data){  
	            if(data != 'true') {
	...
	jQuery.validator.addMethod("check_member_name", function(value, element, params) { 
		var result = true;
		$.ajax({  
	        type:"GET",  
	        url:'<?php echo urlShop('store_member_manage', 'check_member_name');?>',  
	        async:false,  
	        data:{member_name: $('#member_name').val()},  
	        success: function(data){  
	            if(data != 'true') {
	                $.validator.messages.check_member_name = "此会员账号已存在";
	                result = false;
	            }
	        }  
	    });  
