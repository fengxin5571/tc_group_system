...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3>子页面图片</h3>
      <ul class="tab-base">
        <li><a href="index.php?act=subpage_api&op=focus_edit" class="current"><span><?php echo '子页面图片';?></span></a></li>
  ...
  <div class="homepage-focus" id="homepageFocusTab">
    ...
    <!-- 管理理念-->
    <form id="upload_screen_form_index" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?act=web_api&op=screen_pic&flag=1" target="upload_pic">
      <input type="hidden" name="form_submit" value="ok" />
      <input type="hidden" name="web_id" value="150">
      ...
      <div class="full-screen-slides">
        <ul>
          ...
          <li ap="1" screen_id="<?php echo $val['pic_id'];?>" title="可上下拖拽更改显示顺序">
            广告调用
                <a class="del" href="JavaScript:del_index_screen(<?php echo $val['pic_id'];?>);" title="<?php echo $lang['nc_del'];?>">X</a>
            <div class="focus-thumb" onclick="select_index_screen(<?php echo $val['pic_id'];?>);" style="background-color:<?php echo $val['color'];?>;" title="点击编辑选中区域内容">
        ...
        <div class="add-focus"><a class="btn-add-nofloat" href="JavaScript:add_index_screen('pic');"><?php echo '图片调用';?></a>
            <?php if(!empty($output['screen_adv_list']) && is_array($output['screen_adv_list'])){ ?>
            <a class="btn-add-nofloat" href="JavaScript:add_screen('adv');"><?php echo '广告调用';?></a>
            <?php } ?>
            <span class="s-tips"><i></i><?php echo '小提示：单击图片选中修改，拖动可以排序，添加最多不超过6个，保存后生效。';?></span></div>
      </div>
      
     
      <table id="ap_screen_index" class="table tb-type2" style="display:none;">
        <tbody>
          <tr>
      ...
      <table id="upload_screen_index" class="table tb-type2" style="display:none;">
        <tbody>
          <tr>
          ...
          <tr class="noborder">
            ...
            <td class="vatop tips">为确保显示效果正确，请选择W:705px H:506px的清晰图片作为全屏焦点图。</td>
          </tr>
          <tr>
            <td colspan="2" class="required"><label><?php echo '背景颜色'.$lang['nc_colon'];?></label></td>
          </tr>
      ...
      <div class="margintop"><a href="JavaScript:void(0);" onclick="$('#upload_screen_form_index').submit();" class="btn"><span><?php echo $lang['web_config_save'];?></span></a>
    ...
    <form id="upload_screen_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?act=web_api&op=screen_pic" target="upload_pic" style="display:none;">
      <input type="hidden" name="form_submit" value="ok" />
      ...
      <table id="upload_screen" class="table tb-type2" style="display:none;">
        <tbody>
          ...
          <tr class="noborder">
            ...
            <td class="vatop tips">为确保显示效果正确，请选择W:705px H:506p的清晰图片作为全屏焦点图。</td>
          </tr>
          <tr>
            <td colspan="2" class="required"><label><?php echo '背景颜色'.$lang['nc_colon'];?></label></td>
          </tr>
